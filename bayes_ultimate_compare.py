...
def knn_search_pytorch_chunked(feats, k, num_heads=1, chunk_size=4096):
    # --- 🚀 [NEW] Faiss-GPU Acceleration Branch ---
    try:
        import faiss
        ...
        # 使用 StandardGpuResources 进行 GPU 初始化
        res = faiss.StandardGpuResources()
        # IndexFlatIP 对应内积 (特征已归一化，等价于余弦相似度)
        index_cpu = faiss.IndexFlatIP(D_dim)
        # 转移至 GPU 0
        index_gpu = faiss.index_cpu_to_gpu(res, 0, index_cpu)
        index_gpu.add(feats_ready)
        
        # 搜索 k+1 个近邻 (包含自身)
        sims, indices = index_gpu.search(feats_ready, k + 1)
        
        # 返回结果 (Faiss 默认返回的是 Tensor 如果用了 torch_utils)
        # 注意:Faiss 的 IndexFlatIP 返回的是相似度(越大越近)，符合 largest=True 逻辑
        return sims / float(num_heads), indices

    except Exception as e:
        # --- 🛡️ [Fallback] Original PyTorch Implementation ---
        # print(f"Faiss-GPU failed or not found ({e}). Falling back to PyTorch KNN.")
