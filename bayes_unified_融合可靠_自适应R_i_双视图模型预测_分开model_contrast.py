...
# --- 修改部分开始 ---
    ...
    parser.add_argument('--ri_mode', type=str, default='entropy_prior',
                        choices=['confidence', 'entropy_prior', 'uniform'],
                        help='Reliability/prior gate for Bayes fusion. '
                             'confidence keeps the old max-probability ratio; '
                             'entropy_prior uses entropy confidence, KNN/model agreement, '
                             'and an adaptive candidate-prior gate; uniform uses r_i=0.5.')
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    # [Contrast] r_i can be the old confidence ratio or the entropy/prior gate.
    ...
    if model_preds is not None:
        ...
        ri_mode = getattr(args, 'ri_mode', 'entropy_prior')
        if ri_mode == 'confidence':
            conf_knn = p_knn1.max(dim=1)[0]
            conf_model = p_model_raw.max(dim=1)[0]
            r_i = (conf_knn / (conf_knn + conf_model + adap_eps)).unsqueeze(1)
        elif ri_mode == 'uniform':
            r_i = torch.full((N, 1), 0.5, device=p_knn1.device)
        else:
            log_c = math.log(max(int(args.num_classes), 2))
            ent_knn = -(p_knn1.clamp_min(adap_eps) * p_knn1.clamp_min(adap_eps).log()).sum(dim=1) / log_c
            ent_model = -(p_model_effective.clamp_min(adap_eps) * p_model_effective.clamp_min(adap_eps).log()).sum(dim=1) / log_c
            rel_knn = (1.0 - ent_knn).clamp(0.0, 1.0)
            rel_model = (1.0 - ent_model).clamp(0.0, 1.0)
            agreement = (p_knn1 * p_model_effective).sum(dim=1).clamp(0.0, 1.0)
            agree_boost = 0.5 + 0.5 * agreement
            score_knn = rel_knn * agree_boost
            score_model = rel_model * agree_boost
            r_i = (score_knn / (score_knn + score_model + adap_eps)).unsqueeze(1)
    else:
    ...
    with torch.no_grad():
        ...
        logger.info(f"📊 [Stage 2.5] Reliability Scores (ri_mode={getattr(args, 'ri_mode', 'entropy_prior')})")
    ...
    # Candidate prior can be hard, removed, or adaptively relaxed by the fused evidence.
    ...
    elif getattr(args, 'ri_mode', 'entropy_prior') == 'entropy_prior':
        raw_belief_for_gate = r_i * p_knn1 + (1.0 - r_i) * p_model_effective
        prior_gate = (raw_belief_for_gate * omega).sum(dim=1, keepdim=True).clamp(0.0, 1.0)
        adaptive_omega = prior_gate * omega + (1.0 - prior_gate)
        prior_effective = (p_model_effective * adaptive_omega) / ((p_model_effective * adaptive_omega).sum(dim=1, keepdim=True) + adap_eps)
        with torch.no_grad():
            logger.info(f"   ├─ adaptive prior gate: mean={prior_gate.mean().item():.4f} "
                        f"std={prior_gate.std().item():.4f} min={prior_gate.min().item():.4f} "
                        f"max={prior_gate.max().item():.4f}")
    else:
...
class SoftMatchWeightManager:
    """Deprecated in this contrast script. Kept only for checkpoint compatibility."""
    def __init__(self, *args, **kwargs):
        pass
    def __call__(self, preds, index, return_stats=False):
        weights = torch.ones(preds.shape[0], device=preds.device)
        return (weights.detach(), 1.0, 0.0) if return_stats else weights.detach()
...
def train_unified_single_stream(args, encoder, classifier, device,
                                ...
                                proto_manager=None):
    """
    Contrast version: unified single-stream training without the unused
    SoftMatch weighting path on unreliable samples.
    """
    encoder.train()
    classifier.train()
    scaler = GradScaler()
    ...
    for batch_data in unified_loader:
        ...
        # 生成伪标签(使用弱增强)
            with torch.no_grad(), autocast():
                ...
                # No SoftMatch in the contrast script. Keep this path explicit
                # and deterministic: unreliable samples use the calibrated soft
                # target with unit consistency weight.
                consensus_mask = (model_pl[unrel_indices] == knn_pl[unrel_indices])
                final_weights = torch.ones(B_u, device=device, dtype=target.dtype)
                final_weights[~consensus_mask] = 1.0

            with autocast():
                inputs_forward.append(unrel_strong)
                mix_target_self = target
...
def run_single_experiment(args):
    ...
    for epoch in range(total_epochs):
        ...
        avg_sm_weight = train_unified_single_stream(
            ...
            rebalance_factor, proto_manager
        )
