...
SCRIPT="${SCRIPT:-/home/c201/公共/whm/PALS-SOFT/双视图单视图实验结果/experiments/nse_mvp_source_writeback_20260514/train_nse_source_mvp_family_20260520.py}"
DATA_ROOT="${DATA_ROOT:-${PROJECT_ROOT}/data}"
OUT="${OUT:-/home/c201/公共/whm/PALS-SOFT/双视图单视图实验结果/results/nse_persistent_state_five_20260520}"
...
run_exp "FREDIS_PSS" \
  --source_update_mode add_remove \
  --source_update_scope all \
  --source_update_threshold 0.85 \
  --source_remove_threshold 0.05

run_exp "IRNet_PSS" \
  --source_update_mode hard_insert \
  --source_update_scope unreliable_highconf \
  --source_update_threshold 0.85

run_exp "PALS_SARI_PSS" \
  --source_update_mode hard_insert \
...
echo "[launcher] completed c201 persistent-state suite on gpu ${GPU_ID}" | tee -a "${LAUNCH_LOG}"
