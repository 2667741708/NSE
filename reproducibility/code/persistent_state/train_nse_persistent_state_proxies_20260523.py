...
# Created: 2026-05-13
# Purpose: test extractive fixed-source training against destructive weak-source write-back variants.
# -*- coding: utf-8 -*-
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='NSE MVP: fixed-source extraction vs destructive source write-back diagnostics')
    # 基本设置
...
# --- 修改部分开始 ---
    ...
    parser.add_argument('--daes_entropy_coeff', type=float, default=0.5,
                        help='[Topology-DAES] Entropy-adaptive temperature coefficient. Set 0.0 for fixed-temperature DAES.')
    ...
    parser.add_argument('--source_update_mode', type=str, default='none',
                        choices=['none', 'hard_insert', 'hard_remove', 'add_remove', 'mix', 'soft_evidence', 'replace', 'topk_reconstruct',
                                 'fredis_move', 'irnet_correct', 'pals_augment', 'pico_soft_target'],
                        help='[NSE-MVP] Source update mode. none is NSE; hard_insert inserts evidence argmax; hard_remove removes low-confidence source labels; add_remove combines remove+insert; mix blends with evidence one-hot; soft_evidence blends with the full evidence distribution; replace overwrites the working source; topk_reconstruct rebuilds support from top-k evidence scores; fredis_move/irnet_correct/pals_augment/pico_soft_target are prior-aligned persistent-state proxies.')
    parser.add_argument('--source_update_evidence', type=str, default='model',
                        choices=['model', 'p2'],
                        help='[NSE-MVP] Evidence used by source-update operators. model uses classifier probabilities; p2 uses final second-pass NSE evidence.')
    parser.add_argument('--source_update_scope', type=str, default='none', choices=['none', 'all', 'all_highconf', 'unreliable_highconf'],
                        help='[NSE-MVP] Write-back scope. all updates every sample; all_highconf writes all confident model predictions; unreliable_highconf writes only samples outside the active training set.')
    parser.add_argument('--source_update_threshold', type=float, default=0.65,
                        help='[NSE-MVP] Fixed confidence threshold for write-back variants.')
    parser.add_argument('--source_update_schedule', type=str, default='fixed', choices=['fixed', 'pals_linear', 'linear', 'high_linear'],
                        help='[NSE-MVP] fixed uses source_update_threshold; pals_linear uses 0.45 - 0.10 * progress after warmup.')
    parser.add_argument('--source_update_threshold_start', type=float, default=0.95,
                        help='[NSE-MVP] Start value for linear source-update threshold schedules.')
    parser.add_argument('--source_update_threshold_end', type=float, default=0.85,
                        help='[NSE-MVP] End value for linear source-update threshold schedules.')
    parser.add_argument('--source_update_alpha', type=float, default=0.3,
                        help='[NSE-MVP] Blend strength for mix write-back.')
    parser.add_argument('--source_update_start_epoch', type=int, default=-1,
                        help='[NSE-MVP] First epoch index (1-based) allowed to write back. -1 uses model_warmup_epochs + 1.')
    parser.add_argument('--source_remove_threshold', type=float, default=0.05,
                        help='[NSE-MVP] Low-confidence threshold for hard_remove/add_remove candidate deletion.')
    parser.add_argument('--source_insert_margin', type=float, default=0.0,
                        help='[NSE-MVP] For hard insert/add-remove, require the top non-candidate score to exceed the best current candidate score by this margin.')
    parser.add_argument('--fredis_refine_threshold', type=float, default=1e-2,
                        help='[NSE-MVP] FREDIS-style refinement threshold zeta: add non-candidate label j if f_yhat - f_j <= zeta.')
    parser.add_argument('--fredis_disamb_threshold', type=float, default=0.8,
                        help='[NSE-MVP] FREDIS-style disambiguation threshold zeta_bar: remove candidate label j if f_yhat - f_j >= zeta_bar.')
    parser.add_argument('--fredis_refine_min_conf', type=float, default=0.0,
                        help='[NSE-MVP] Minimum non-candidate score for FREDIS-style refinement.')
    parser.add_argument('--fredis_disamb_max_conf', type=float, default=1.0,
                        help='[NSE-MVP] Maximum candidate score for FREDIS-style disambiguation.')
    parser.add_argument('--fredis_top_non_candidate_only', action='store_true',
                        help='[NSE-MVP] Restrict FREDIS-style refinement to the top non-candidate label per sample.')
    parser.add_argument('--fredis_min_disamb_over_refine', type=float, default=1.0,
                        help='[NSE-MVP] Deterministic proxy for FREDIS sample-size control: keep at most disambiguous_labels / ratio refinement labels. 0 disables this cap.')
    parser.add_argument('--irnet_tau_boundary', type=float, default=0.0,
                        help='[NSE-MVP] IRNet-style noisy detector boundary for tau=max_candidate_score-max_non_candidate_score.')
    parser.add_argument('--irnet_min_non_candidate_conf', type=float, default=0.0,
                        help='[NSE-MVP] Minimum best non-candidate score for IRNet-style correction. 0 follows the tau detector only.')
    parser.add_argument('--pico_candidate_constrained', action='store_true',
                        help='[NSE-MVP] For pico_soft_target, project evidence onto the current candidate support before the moving-average target update.')
    parser.add_argument('--source_reconstruct_k', type=int, default=5,
                        help='[NSE-MVP] Number of labels retained by topk_reconstruct.')
    parser.add_argument('--source_reset_interval', type=int, default=0,
                        help='[NSE-MVP] If >0, reset mutable source to the native source every K epochs before applying write-back. K=10 gives a periodic-reset diagnostic.')
    parser.add_argument('--persistent_promotion_mode', type=str, default='none',
                        choices=['none', 'hard'],
                        help='[NSE-MVP] Persistent sample-state proxy. hard stores high-confidence pseudo-labels and forces those samples into later active training.')
    parser.add_argument('--promotion_scope', type=str, default='none',
                        choices=['none', 'all_highconf', 'unreliable_highconf', 'nse_estimated_noise_highconf'],
                        help='[NSE-MVP] Scope for persistent promotion. nse_estimated_noise_highconf uses the current NSE-selected unreliable/noise set before salvage/persistent promotion.')
    parser.add_argument('--promotion_threshold', type=float, default=0.95,
                        help='[NSE-MVP] Confidence threshold for persistent sample-state promotion.')
    parser.add_argument('--promotion_source', type=str, default='p2',
                        choices=['model', 'p2'],
                        help='[NSE-MVP] Evidence used by persistent promotion. p2 uses final second-pass NSE evidence.')
    parser.add_argument('--promotion_label_space', type=str, default='all',
                        choices=['all', 'non_candidate'],
                        help='[NSE-MVP] Label space for persistent promotion. non_candidate takes argmax over the native source complement.')
...
def train_unified_single_stream(args, encoder, classifier, device,
                                ...
                                logger, num_classes, global_labels, global_is_reliable, knn_pl, model_pl, knn_scores,
    ...
    classifier.train()
    scaler = GradScaler()
    disable_rel_mixup = bool(getattr(args, 'ablate_no_reliable_mixup', False))
    logger.info(f"[AblationCheck] reliable_mixup={not disable_rel_mixup}")
    ...
    for batch_data in unified_loader:
        ...
        is_reliable = global_is_reliable[indices]
        ...
        # Ordinary unreliable samples already receive zero sampler weight in the active-set construction.
        ...
        if reliable_mask.sum() > 0:
            ...
            B_s = rel_weak.size(0)
            if disable_rel_mixup:
                mix_w, mix_l_w = rel_weak, s_labels
                mix_s, mix_l_s = rel_strong, s_labels
            else:
...
def _current_writeback_threshold(args, epoch):
    ...
    if start_epoch < 0:
        start_epoch = int(getattr(args, 'model_warmup_epochs', 20)) + 1
...
def _apply_mvp_source_writeback(args, logger, dataset, epoch, model_scores, active_mask):
...
def run_single_experiment(args):
    ...
    if False: exp_type = "Sup_Only"
    elif False and False: exp_type = "No_Mixup_No_SoftMatch"
    elif args.no_rebalance: exp_type = "No_Rebalance"
    elif False: exp_type = "No_SoftMatch"
    elif False: exp_type = "No_Unrel_Mixup"
    elif args.ablate_no_reliable_mixup: exp_type = "No_Rel_Mixup"
    ...
    for epoch in range(total_epochs):
        ...
        # 5.2 历史一致性权重保留为内部诊断量。
        # 当前采样器将非 reliable 且非 salvage 样本权重置为 0，训练实际只使用 reliable/salvage。
        ...
        dynamic_consistency_weight = unreliable_ratio * (consensus_prop ** 2.0)
        ...
        # 剩下的是不可靠
        final_unreliable_indices = [idx for idx in unreliable_indices.cpu().tolist() if idx not in salvage_set and idx not in persistent_set]

        # --- 统一权重计算 ---
        total_target = len(base_train_ds)
        ordered_indices = persistent_indices + salvage_indices + final_unreliable_indices + real_reliable_indices
        ...
        # B. 不可靠集 (Unreliable) -> 权重 0.0 (此代码默认初始化即为0.0，故主要作为显式占位和标签覆写)
        for idx in final_unreliable_indices:
            sampling_weights_aligned.append(0.0)
            global_labels[idx] = -1
            global_is_reliable[idx] = False
        ...
        logger.info(f" >> [Sampler] Persistent: {len(persistent_indices)} | Salvaged: {len(salvage_indices)} | Reliable: {len(real_reliable_indices)} | Unified Weight: {unified_weight:.2f}x")
        ...
        # 恢复旧版采样语义: 按 salvage -> unreliable -> reliable 的顺序组织索引空间。
        # 这里仍然复用 DataLoader 对象,但让每个 epoch 的 batch 序列贴近旧脚本。
        ...
        avg_sm_weight = train_unified_single_stream(
            ...
            logger, num_classes, global_labels, global_is_reliable, knn_pl, model_pl, knn_scores, dynamic_consistency_weight, 
        ...
        _apply_mvp_source_writeback(args, logger, base_train_ds, epoch, source_scores, active_mask_np)
