...
python bayes_unified_融合可靠_自适应R_i_双视图模型预测_分开model.py \
  ...
  --max_w_model 0.5 \
  --out ./out_ultimate \
  --exp_name bayes_unified_融合可靠_自适应R_i_双视图模型预测_分开model/c100_pr001_nr00_maxw05/refactored_e100_seed123 \
  --seeds 1 2 3  --cuda_dev 0 
...
# --- 修改部分开始 ---
    ...
    # [新增] 共识模式选择 / [NEW] Consensus mode selection
    parser.add_argument('--consensus_mode', type=str, default='model', 
                        choices=['knn', 'model'],
                        help='Consensus mode for fused_blend calculation: "knn" (r_i * p_knn1) or "model" (r_i * p_model_effective).')
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    # fused_blend selection (Comparison Study)
    if getattr(args, 'consensus_mode', 'model') == 'knn':
        fused_blend = r_i * p_knn1 + (1.0 - r_i) * prior_effective
        logger.info("   ├─ Fusion logic: Consensus Mode = KNN (r_i * p_knn1)")
    else:
        fused_blend = r_i * p_model_effective + (1.0 - r_i) * prior_effective
        logger.info("   ├─ Fusion logic: Consensus Mode = Model (r_i * p_model_effective)")
    ...
    with torch.no_grad():
        ...
        omega_support = (omega > 0).float().sum(dim=1).float().mean().item()
        model_top = p_model_raw.argmax(dim=1)
        effective_top = p_model_effective.argmax(dim=1)
        prior_top = prior_effective.argmax(dim=1)
        model_outside_prior_rate = (omega.gather(1, model_top.unsqueeze(1)).squeeze(1) <= 0).float().mean().item()
        effective_outside_prior_rate = (omega.gather(1, effective_top.unsqueeze(1)).squeeze(1) <= 0).float().mean().item()
        prior_constraint_change_rate = (effective_top != prior_top).float().mean().item()
        model_top1_acc = (model_top == clean_labels).float().mean().item()
        prior_effective_top1_acc = (prior_top == clean_labels).float().mean().item()
        logger.info(f"📊 [Stage 2.8] Unified Bayesian Fusion (Confidence-based r_i)")
        logger.info(f"   ├─ omega support: mean={omega_support:.2f} active classes/sample")
    ...
    logger.info(f"{'='*80}\n")
    # [Monitoring/监测] Store scalar diagnostics for epoch_diagnostics.csv; no training behavior is changed.
    args.last_selection_metrics = {
        'stage0_selected': int(size_init),
        'stage0_acc': acc_init * 100,
        'stage0_overall_acc': overall_acc_init * 100,
        'stage1_selected': int(size_st1),
        'stage1_acc': acc_st1 * 100,
        'stage1_overall_acc': overall_acc_st1 * 100,
        'w_model': float(w_model),
        'ri_mean': r_i.mean().item(),
        'ri_std': r_i.std().item(),
        'ri_min': r_i.min().item(),
        'ri_max': r_i.max().item(),
        'omega_support_mean': omega_support,
        'model_top1_acc': model_top1_acc * 100,
        'model_outside_prior_rate': model_outside_prior_rate * 100,
        'effective_outside_prior_rate': effective_outside_prior_rate * 100,
        'prior_constraint_change_rate': prior_constraint_change_rate * 100,
        'prior_effective_top1_acc': prior_effective_top1_acc * 100,
        'stage28_selected': int(size_s28),
        'stage28_acc': acc_s28 * 100,
        'stage28_overall_acc': overall_acc_s28 * 100,
        'stage2_selected': int(size_rel_final),
        'stage2_acc': acc_rel_final * 100,
        'stage2_overall_acc': overall_acc_final * 100,
    }
...
def run_single_experiment(args):
    ...
    metrics_csv_path = os.path.join(log_dir, "epoch_diagnostics.csv")
    metrics_fields = [
        'epoch', 'test_acc', 'best_acc', 'lr', 'unreliable_ratio', 'consensus_prop',
        'dynamic_consistency_weight', 'salvage_count', 'salvage_acc', 'reliable_count',
        'avg_softmatch_weight', 'stage0_selected', 'stage0_acc', 'stage0_overall_acc',
        'stage1_selected', 'stage1_acc', 'stage1_overall_acc', 'w_model', 'ri_mean',
        'ri_std', 'ri_min', 'ri_max', 'omega_support_mean', 'model_top1_acc',
        'model_outside_prior_rate', 'effective_outside_prior_rate',
        'prior_constraint_change_rate', 'prior_effective_top1_acc', 'stage28_selected',
        'stage28_acc', 'stage28_overall_acc', 'stage2_selected', 'stage2_acc',
        'stage2_overall_acc'
    ]
    ...
    for epoch in range(total_epochs):
        ...
        ordered_indices = salvage_indices + final_unreliable_indices + real_reliable_indices
        ...
        metrics_row = {k: '' for k in metrics_fields}
        metrics_row.update(getattr(args, 'last_selection_metrics', {}))
        metrics_row.update({
            'epoch': epoch + 1,
            'test_acc': test_acc,
            ...
            'dynamic_consistency_weight': dynamic_consistency_weight,
            'salvage_count': int(salvage_mask.sum().item()) if salvage_mask is not None else 0,
            'salvage_acc': acc_tri,
            'reliable_count': len(real_reliable_indices),
            'avg_softmatch_weight': avg_sm_weight,
        })
        with open(metrics_csv_path, 'a', newline='') as f:
            csv.DictWriter(f, fieldnames=metrics_fields).writerow(metrics_row)
